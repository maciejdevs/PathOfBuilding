...
-- Opens the item pricing popup
function TradeQueryClass:PriceSlotItem()
	self.tradeQueryGenerator = new("TradeQueryGenerator", self)
	...
	top_pane_alignment_ref = {"TOPLEFT", self.controls.poesessidButton, "BOTTOMLEFT"}
	for _, slotName in ipairs(baseSlots) do
		self:PriceItemSlotRowDisplay(cnt, {name = slotName}, top_pane_alignment_ref, top_pane_alignment_width, top_pane_alignment_height, row_height)
		top_pane_alignment_ref = {"TOPLEFT", self.controls["name"..cnt], "BOTTOMLEFT"}
		cnt = cnt + 1
	end
	local activeSocketList = { }
	...
	table.sort(activeSocketList)
	for _, nodeId in pairs(activeSocketList) do
		self:PriceItemSlotRowDisplay(cnt, {name = self.itemsTab.sockets[nodeId].label, ref = nodeId}, top_pane_alignment_ref, top_pane_alignment_width, top_pane_alignment_height, row_height)
		top_pane_alignment_ref = {"TOPLEFT", self.controls["name"..cnt], "BOTTOMLEFT"}
		cnt = cnt + 1
	end
	self.controls.fullPrice = new("LabelControl", nil, -3, pane_height - 58, pane_width - 256, row_height, "")
...
function TradeQueryClass:PriceItemSlotRowDisplay(str_cnt, slotTbl, top_pane_alignment_ref, top_pane_alignment_width, top_pane_alignment_height, row_height)
	...
	controls["bestButton"..str_cnt] = new("ButtonControl", {"TOPLEFT",controls["name"..str_cnt],"TOPLEFT"}, 100 + 8, 0, 80, row_height, "Find price", function()
		self.tradeQueryGenerator:GetItemQuery(slotTbl.ref and self.itemsTab.sockets[slotTbl.ref] or self.itemsTab.slots[slotTbl.name], { slotTbl = slotTbl, controls = controls, str_cnt = str_cnt }, function(context, query, errMsg)
			if errMsg then
